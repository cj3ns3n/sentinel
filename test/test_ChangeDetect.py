...
def changeDetect(image1, image2, tester, detectors):
  changeDetect = ChangeDetect(setActiveCallback, detectors)
...
class TestChangeDetect(unittest.TestCase):
  ...
  def test_diff(self):
    ...
    structuralSimilarityChangeDetect = ChangeDetectStructuralSimilarity()
    yoloChangeDetect = ChangeDetectYolo()
    detectors = [structuralSimilarityChangeDetect, yoloChangeDetect]

    image1 = cv2.imread('test/img1.jpg')
    image2 = cv2.imread('test/img2.jpg')
    boxedImg = changeDetect(image1, image2, self, detectors)
